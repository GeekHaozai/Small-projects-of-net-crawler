class Solution(object):
    def matrixSum(self, nums):
        ...
        son_list=[]
        res=0
        for j in range(len(nums[0])):#执行j次,直到所有元素为0
            for m in range(len(nums)):#循环每个子数列
                #每次找到子数组中最大的
                each_max=self.compare_num(nums[m])
                son_list.append(each_max)
            res += self.compare_num(son_list)
            son_list=[]
        return res

    def compare_num(self,num_list):
        max = 0
